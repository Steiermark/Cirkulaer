...
suffix (2 rows vs 50+ of any Roland vs 6 exact, measured 2026-09-11). Then it walks a
ladder until dba returns rows: `assessment.search_terms` in order, which the vision prompt
asks for as "what a Danish seller would title the ad" — "PH 5 pendel sort hvid" finds
look-alikes where the object name "Pendellampe" found seven strangers — then the sale
query, the subcategory, the object name. Each rung is one ~1s GET. The sale query still feeds the
